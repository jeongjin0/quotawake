...
- **Structure**: the selected provider's full readout, opening with the same answer-first order as the overview: a provider-scoped Next Reset hero leads (same 30pt treatment as the overview hero, subline "5h window · at HH:MM"; with no observed reset candidate it keeps its place and shows the overview hero's waiting empty state — "Waiting for a quota signal" / "Reload to check now" — instead of disappearing), then the provider header (identity mark, name, status dot + status text), then one card holding the 5h window and Weekly Limit sections in the same label/value/bar rhythm (prominent variant: 14pt monospaced-digit values, 6pt bars; the 5h reset footnote is omitted because the hero above states it), then quiet Source / Confidence / Last run meta rows, then `RECENT ACTIVITY` filtered to this provider.
