...
- **Stable cross-platform release: NO-GO.** Native compiler gates now pass, but
  Linux and Windows have not passed package, service-lifecycle, and clean-machine
  runtime gates.
...
- **Private technical source prerelease: CONDITIONAL GO.** Label it as a
  developer preview and state the exact CI evidence. Do not publish Linux or
  Windows binaries until packaging and clean-machine gates pass.

## Confidence

| Surface | Confidence | Basis |
| --- | ---: | --- |
| Reset-aware core and provider safety boundaries | 88% | 166 automated tests pass on macOS and Ubuntu; the fake-provider live-smoke self-test passes on macOS |
...
| Linux manual CLI | 70% | Ubuntu 24.04 build, all 166 tests, and CLI smoke pass; archive and clean-runtime execution are unverified |
| Linux daemon | 40% | Scheduler tests pass on Ubuntu, but `systemd-logind` and the full `systemd --user` lifecycle have not run in a VM |
| Windows manual CLI | 45% | Native Swift 6.3.3 build and CLI help smoke pass; tests, packaging, and clean-runtime execution are unverified |
...
- **Portability: conditional compiler pass, fail for general availability.**
  Native Linux and Windows builds pass, but no artifact has passed the complete
  package-to-service path on either OS.
...
| R2 | P0 | Linux and Windows package/service lifecycle gates have not run | Build archives in native CI, execute them on clean machines, and test install/login/start/status/stop/uninstall before publishing platform assets |
...
| R12 | P2 | Linux provider launch currently depends on `/bin/bash`, and the macOS/Linux Swift setup action still emits a Node.js runtime deprecation warning | Document/package the Bash dependency, replace the descriptor-cleanup trampoline with a native spawn layer, and update the action when its maintained release removes the warning |
...
- [GitHub Actions run 30566908704](https://github.com/jeongjin0/quotawake/actions/runs/30566908704)
  passed at commit `8b234c7`: macOS 15 and Ubuntu 24.04 each built the CLI,
  passed all 166 tests, and passed CLI smoke; Windows built the CLI and passed
  CLI smoke with Swift 6.3.3.
- No live Claude or Codex readiness prompt was sent during this assessment.
- No local Linux VM or Windows machine was used. GitHub-hosted native runners
  covered compiler/test smoke only, not interactive sessions or service lifecycle.
